        ...
        public static IServiceCollection Populate(IServiceCollection services, RegistrationStrategy strategy, string filePath, string memberName, int lineNumber)
        {
            switch (lineNumber)
            {
                case 14:
                    ...
                    strategy.Apply(services, ServiceDescriptor.Describe(typeof(RootDependencyProject.IRequestHandler<, >).MakeGenericType(Assembly.Load(Dependency3ProjectVersion0000CultureneutralPublicKeyTokennull).GetType("Dependency1Project.Request3"), Assembly.Load(Dependency3ProjectVersion0000CultureneutralPublicKeyTokennull).GetType("Dependency1Project.Response3")), Assembly.Load(Dependency3ProjectVersion0000CultureneutralPublicKeyTokennull).GetType("Dependency1Project.RequestHandler3"), ServiceLifetime.Singleton));
