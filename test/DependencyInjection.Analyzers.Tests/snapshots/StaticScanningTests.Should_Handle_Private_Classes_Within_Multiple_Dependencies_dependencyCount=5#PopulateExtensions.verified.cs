        ...
        public static IServiceCollection Populate(IServiceCollection services, RegistrationStrategy strategy, AssemblyLoadContext context, string filePath, string memberName, int lineNumber)
        {
            switch (lineNumber)
            {
                case 14:
                    ...
                    strategy.Apply(services, ServiceDescriptor.Describe(typeof(RootDependencyProject.IService), context.LoadFromAssemblyName(Dependency3ProjectVersion0000CultureneutralPublicKeyTokennull).GetType("Dependency1Project.Service3"), ServiceLifetime.Singleton));
                    strategy.Apply(services, ServiceDescriptor.Describe(typeof(RootDependencyProject.IService), context.LoadFromAssemblyName(Dependency4ProjectVersion0000CultureneutralPublicKeyTokennull).GetType("Dependency1Project.Service4"), ServiceLifetime.Singleton));
                    break;
            }
