...
public class InjectableMethodBuilder<T, T2> : InjectableMethodBuilderBase
{
    ...
    /// <returns></returns>
    public InjectableMethodBuilder<T, T2, TNext> WithParameter<TNext>() => new(Container, MethodNames);
    ...
    /// <returns></returns>
    public InjectableMethodBuilder<T, T2> ForMethod(string methodName) => new(Container, MethodNames.Add(methodName));
